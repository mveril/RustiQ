...
mod tests {
    ...
    fn test_geometry_parsing_and_conversion() {
        // Exemple de chaîne XYZ pour H2 centrée en Angstrom
        let xyz_data = "\
2
...
";
        ...
        // Création d'un Cursor pour simuler un lecteur de fichier
        let cursor = Cursor::new(xyz_data);

        // Parsing de la géométrie avec display_unit = Angstrom et internal_unit = Bohr
        let geometry = Geometry::from_reader(cursor, Some(Units::Bohr), Some(Units::Angstrom))
            .expect("Failed to parse Geometry");

        // Vérification du nombre d'atomes
        assert_eq!(geometry.atoms.len(), 2);

        // Vérification des coordonnées converties
        // 1 Angstrom = 1 / 0.52917721092 Bohr ≈ 1.8897259886 Bohr
    ...
    fn test_geometry_display() {
        // Création d'une géométrie simple
        ...
        // Vérification de la sortie de Display
    ...
    fn test_nucl_repulsion_h2() {
        // Création d'une géométrie simple pour la molécule H2
        ...
        // Calcul de la répulsion noyau-noyau pour H2
        let e_nuc_nuc = geometry.nucl_repulsion();

        // Valeur attendue pour H2 à cette distance
        // La distance en Bohr est ~1.40 Bohr (0.74 Ångström)
        let expected_e_nuc_nuc = 1.0 / 1.40; // Z_H * Z_H / R_HH

        // Vérification que l'énergie calculée est proche de la valeur attendue
        assert!(
    ...
    fn test_nucl_repulsion_h3() {
        // Création d'une géométrie pour un système hypothétique H3 linéaire
        ...
        // Calcul de la répulsion noyau-noyau pour H3
        let e_nuc_nuc = geometry.nucl_repulsion();

        // Les distances sont toutes 1 Bohr ici
    ...
    fn test_nucl_repulsion_he_h2() {
        // Création d'une géométrie pour un système avec un atome d'He et un H2
        ...
        // Calcul de la répulsion noyau-noyau
        let e_nuc_nuc = geometry.nucl_repulsion();

        // Les distances sont He-H1 = 2 Bohr, He-H2 = 3 Bohr, H1-H2 = 1 Bohr
